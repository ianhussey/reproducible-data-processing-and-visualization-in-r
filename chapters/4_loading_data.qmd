...
Note that this code uses the 'pipe' (`%>%`), which we cover in more detail in a [later chapter](the_pipe_and_renaming.qmd). You don't need to understand how it works yet, just the output that it creates.
...
Note that you have to either first load the {read_xl} package with `library(readxl)`, or specify which package the function comes from with `?readxl::read_excel()`. See the section on dependencies in the chapter on [Fundamentals](fundamentals.qmd) to refresh your knowledge.
:::
