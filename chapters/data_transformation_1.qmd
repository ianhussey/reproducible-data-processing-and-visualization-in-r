...
-   add a broken qualtrics file example where the text responses don't match the number responses
...
- `&` : AND, i.e., meeting both criterion. 
  - `dat %>% filter(gender == "male" & age == 22)` returns participants whose gender is set to "male" and age is 22. 
- `|` : OR, i.e., meeting either criterion. 
  - `dat %>% filter(gender == "male" | age == 22)` returns participants whose gender is set to "male" or their age is 22. Female participants aged 22 will be returned, as will male participants aged 26 etc. 
  - `dat %>% filter(gender == "male" | gender == "female)` returns participants whose gender is set to "male" or "female" but not other responses or NA.
...
### Working with strings, case, characters etc.
...
Complete the interactive `select()` and `relocate()` exercises [here](https://errors.shinyapps.io/dplyr-learnr/#section-dplyrselect). This web app is written in the {shiny} package and allows you to write and run code in your web browser.
