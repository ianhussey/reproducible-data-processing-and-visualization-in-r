...
# You asked for "readme.md" and "licence".
...
if (length(license_paths) > 0) {
  ...
  results <- dplyr::bind_rows(
    results,
    mk_test("License name canonical (LICENSE)", canonical,
...
# - .png only under code/plots (and must exist there)
# - .rds only under code/models (and must exist there)
# - .qmd only under code/ (and must exist there)
# - .Rmd should not appear anywhere
# - .R   should not appear anywhere (you did not show any .R files)
# - .csv only under data/processed, data/raw, data/results (and must exist in those as shown)
# - .xlsx only under data/processed and data/raw (and must exist in those as shown)
# (You also have .html in code/; we will restrict .html to code/.)

constraints <- list(
  png  = list(must_be_in = c("code/plots"), must_exist_in_each = TRUE, forbidden_elsewhere = TRUE),
  rds  = list(must_be_in = c("code/models"), must_exist_in_each = TRUE, forbidden_elsewhere = TRUE),
  qmd  = list(must_be_in = c("code"), must_exist_in_each = TRUE, forbidden_elsewhere = TRUE),
  Rmd  = list(must_be_in = character(0), must_exist_in_each = FALSE, forbidden_elsewhere = TRUE),
  R    = list(must_be_in = character(0), must_exist_in_each = FALSE, forbidden_elsewhere = TRUE),
  csv  = list(must_be_in = c("data/processed","data/raw","data/results"), must_exist_in_each = TRUE, forbidden_elsewhere = TRUE),
  xlsx = list(must_be_in = c("data/processed","data/raw"), must_exist_in_each = TRUE, forbidden_elsewhere = TRUE)
)

# Additionally restrict html to code/
html_files <- list_ext("html")
if (length(html_files)) {
  html_ok <- all(starts_with_any(html_files, "code"))
  results <- dplyr::bind_rows(results, mk_test(
    "All .html files are under code/",
    html_ok,
    if (!html_ok) paste0("Move .html files to code/. Offenders: ",
                         paste(fs::path_rel(html_files[!starts_with_any(html_files, "code")], start = project_root), collapse = "; ")) else ""
  ))
}

check_constraint <- function(ext, rule) {
  files <- list_ext(ext)
  ...
  if (isTRUE(rule$must_exist_in_each) && length(rule$must_be_in) > 0) {
    for (dir_req in rule$must_be_in) {
      # any file with this ext under required dir?
  ...
  if (isTRUE(rule$forbidden_elsewhere)) {
    if (length(files)) {
      allowed_ok <- if (length(rule$must_be_in) == 0) {
        # No locations allowed -> none should exist anywhere
        length(files) == 0
      } else {
        # all files must reside under one of allowed prefixes
      ...
      results <<- dplyr::bind_rows(results, mk_test(
        paste0("All .", ext, if (length(rule$must_be_in)) paste0(" files reside in ", paste(rule$must_be_in, collapse = ", ")) else " files are absent"),
        allowed_ok,
        if (!allowed_ok) paste0("Move/remove: ", paste(offenders, collapse = "; ")) else ""
      ))
    } else {
      # If they must exist in specific dirs, absence will already be flagged above
      if (isFALSE(rule$must_exist_in_each)) {
        results <<- dplyr::bind_rows(results, mk_test(
...
if (enable_hygiene) {
  ...
  # .html only in code/ already checked above
  ...
  # Flag very large files (>50MB) not in communications/ (encourage Git LFS)
